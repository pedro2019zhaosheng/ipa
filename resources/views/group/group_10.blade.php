...
@section('content')
    <section class="content-header">
        <h1 class="pull-left">开启后，用户可通过@被投诉人+%投诉+投诉内容，进行投诉</h1>
        <h1 class="pull-right">
    ...
    <div class="content">
        ...
        {!! Form::open(['url' => '/group/addauto','files' => true, 'method'=>'POST']) !!}
          ...
          {{--<input type="checkbox" name="mm[]" id="boxid" onclick="setAllNo()" />全选/全不选--}}
            <input type="hidden" name="robot_type" value="7">
        ...
        <div class="box box-primary">
            <div class="box-body">
                <label class="checkbox-inline">
                    {{--<td ><button style="margin-right: 40px"><a id="links" style="color:grey" href ="/group/groupComplain?status=1">开启</a></button></td>--}}
                    {{--<td ><button style="margin-right: 40px"><a id="links" style="color:grey" href ="/group/groupComplain?status=0">关闭</a></button></td>--}}
        ...
        <div class="box box-primary">
            ...
            <table class="table table-responsive" id="games-table">
                ...
                @foreach($replys as $reply)
                    <tr>
                        <td>{!! $reply['id'] !!}</td>
                        <td>{!! $reply['complained_nickname'] !!}</td>
                        <td>{!! $reply['complain_nickname'] !!}</td>
                        <td>{!! $reply['complain_date'] !!}</td>

                        <td>{!! $reply['group_desc'] !!}</td>
                        <td>{!! $reply->status == 0 ? '踢出' : '正常' !!}</td>
                        {{--<td><img class='
    ...
    <script type="text/javascript">
        function aa(){
            ...
            var url = '/group/saveRobotId?robot_id='+check_val+'&save_type=10';
            $("#sampling").val(url);
            if(check_val==''){
                check_val = 0;
            }
        ...
        function group(){
            ...
            $.ajax({
                ...
                data: {"group_id":check_val, '_token':'{{csrf_token()}}','key':key,'group_type':3,'no_check_id':ms},

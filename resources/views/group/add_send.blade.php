...
@section('content')
    ...
    <div class="content">
        ...
        <div class="box box-primary">
            ...
            {!! Form::open(['url' => '/group/addSend','files' => true, 'method'=>'POST']) !!}
            ...
            <div class="box box-primary">
                <div class="box-body">
                    ...
                    <div class="box box-primary">
                        <div class="box-body">
                            发送时间：</br></br>
                            <label><input type="radio" value="0" @if($wxdata->is_repeat==0)checked @endif name="is_repeat">不重复</label>
                            <label><input type="radio" value="1"  @if($wxdata->is_repeat==1)checked @endif  name="is_repeat">重复</label>
                            </br>
                            </br>
                            @foreach($week as $k=>$v)
                                <label class="checkbox-inline">
                                    <input type="checkbox" @if($v['status']==1) checked @endif  onclick="" id="'group" name="week[]" value="{{$v['id']}}" />{{$v['name']}}
                                </label>
                            @endforeach
                            </br>
                                </br>
                            <div>发送时间：
                            <select id="pro"  name ="time" class="pro" style="width:130px;">
                                <option name ="time[]" value ="-99" pro="0">==请选择==</option>
                                @foreach($time as $k=>$v)
                                    <option name ="time[]" @if($wxdata->time==$v['name'])selected @endif  value="{{$v['name']}}">{{$v['name']}}</option>
                                @endforeach
                            </select>

                            <select id="pro"  name ="minute" class="pro" style="width:130px;">
                                <option name ="minute[]" value ="-99" pro="0">==请选择==</option>
                                @foreach($minute as $k=>$v)
                                    <option name ="minute[]" @if($wxdata->minute==$v['name'])selected @endif  value="{{$v['name']}}">{{$v['name']}}</option>
                                @endforeach
                            </select>
                            </div>
    ...
    <script type="text/javascript">
        function aa(){
            ...
            var url = '/group/saveRobotId?robot_id='+check_val+'&save_type=6';
            $("#sampling").val(url);
            if(check_val==''){
                check_val = 0;
            }

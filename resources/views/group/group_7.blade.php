        ...
        <div class="box box-primary">
            <div class="box-body">
                <label class="checkbox-inline">
                    <td ><button style="margin-right: 40px"><a id="links" style="color:grey" href ="/group/groupSign?status=1">开启</a></button></td>
                    <td ><button style="margin-right: 40px"><a id="links" style="color:grey" href ="/group/groupSign?status=0"  onclick = "return confirm('正在执行的进程当天不会被关闭，是否确定要关闭?')">关闭</a></button></td>

                    <td ><button style="margin-right: 40px"><a id="links" style="color:grey" href ="/group/addSign?robot_id={{$wxdata->robot_id}}">新建签到</a></button></td>
            ...
            <table class="table table-responsive" id="games-table">
                ...
                @foreach($replys as $reply)
                    <tr>
                        ...
                        <td>
                            {!! $reply['start_date'] !!}-{{$reply['end_date']}}
                            </br>
                            {!! $reply['start_time'] !!}-{{$reply['end_time']}}
                        </td>
                        <td>{!! $reply['sign_count'] !!}</td>
                        ...
                        <td>
                            {!! Form::open(['url' => ['group/destroySign?id='.$reply->id ], 'method' => 'delete']) !!}
                            <div class='btn-group'>
                                <a href="/group/addSign?robot_id={{$reply->robot_id}}&id={{$reply->id}}&is_first=1" class='btn btn-default btn-xs'>修改</a>

...
@section('content')
    ...
    <div class="content">
        ...
        <div class="box box-primary">
            <div class="box-body">
                ...
                @foreach($group as $k=>$v)
                    <label class="checkbox-inline">
                        <input onclick="group()" type="checkbox" @if($v['status']==1) checked @endif  onclick="" id="'group" name="group[]" value="{{$v['id']}}" />{{$v['sname']}}
                    </label>
                @endforeach
            </div>
        </div>
        ...
        <div class="box box-primary">
            <div class="box-body">
                <label class="checkbox-inline">
                    <td ><button style="margin-right: 40px"><a id="links" style="color:grey" href ="/group/groupMember?is_kick=1">踢出</a></button></td>
                    <td ><button style="margin-right: 40px"><a id="links" style="color:grey" href ="/group/groupMember?is_block=1">踢出并拉黑</a></button></td>
                    <td ><button style="margin-right: 40px"><a id="links" style="color:grey" href ="/group/groupMember?is_admin=1">设为管理员</a></button></td>
        ...
        <div class="box box-primary">
            ...
            <table class="table table-responsive" id="games-table">
                ...
                @foreach($replys as $reply)
                    <tr>
                        <td>{!! $reply['id'] !!}</td>
                        <td>{!! $reply['nickanme'] !!}</td>
                        <td>{!! $reply['off_days'] !!}</td>
                        <td>{!! $reply['join_group_date'] !!}</td>

                        <td>{!! $reply['last_msg_date'] !!}</td>
                        <td>{!! $reply['invite_num'] !!}</td>
                        <td>{!! $reply['invite_retain_num'] !!}</td>
                        <td>{!! $reply['msg_num'] !!}</td>
                        <td>{!! $reply['complaints_num'] !!}</td>
                        {{--<td><img class='
    ...
    <script type="text/javascript">
        function aa(){
            ...
            var url = '/group/saveRobotId?robot_id='+check_val+'&save_type=8';
            $("#sampling").val(url);
            if(check_val==''){
                check_val = 0;
            }
        ...
        function group(){
            ...
            $.ajax({
                ...
                data: {"group_id":check_val, '_token':'{{csrf_token()}}','key':key,'group_type':1,'no_check_id':ms},
